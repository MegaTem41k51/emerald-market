...
// =====================================================
// API: ЗАГРУЗКА ИНВЕНТАРЯ ПО STEAM ID (из Trade URL)
// =====================================================
...
app.post('/api/get-inventory-by-id', async (req, res) => {
    ...
    try {
        ...
        if (cachedItems) {
            ...
            return res.json({ success: true, cached: true, items: cachedItems });
        }
        ...
        console.log(`📦 Запрашиваем CS2 inventory по ID: ${steamId}`);
        ...
        if (response.status === 429) {
            return res.status(429).json({
                error: 'Steam временно ограничил запросы к инвентарю. Подождите немного.'
            });
        }
        ...
        if (!inventory || inventory.success === false) {
            return res.status(400).json({
                error: 'Steam не разрешил получить инвентарь. Проверьте настройки приватности.'
            });
        }
        ...
        for (const asset of inventory.assets) {
            ...
            // Исключаем мусор
            if (lowerName.includes('case') || lowerName.includes('crate') || 
                lowerName.includes('capsule') || lowerName.includes('sticker') || 
                lowerName.includes('graffiti') || lowerName.includes('music kit') || 
            ...
            if (lowerName.includes('knife') || lowerName.includes('karambit') || 
                lowerName.includes('bayonet') || lowerName.includes('butterfly') || 
                lowerName.includes('gloves')) {
                isSkin = true;
            }
        ...
        console.log(`✅ ${steamId}: найдено ${items.length} скинов`);
        
        return res.json({ success: true, cached: false, items: items });
        
    } catch (error) {
        console.error('❌ Ошибка загрузки инвентаря по ID:', error.message);
        return res.status(500).json({
